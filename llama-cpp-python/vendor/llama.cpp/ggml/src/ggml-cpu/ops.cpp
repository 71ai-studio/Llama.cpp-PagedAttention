...
// Variant for F16 src0 — converts F16→F32 per row then quantizes to dst type
template<typename idx_t>
        ...
        for (int64_t i02 = 0; i02 < ne02; ++i02) {
            for (int64_t i = ir0; i < ir1; ++i) {
                ...
                for (int64_t j = 0; j < nc; ++j) {
                    tmp[j] = GGML_CPU_FP16_TO_FP32(src_row[j]);
                }

                from_float(tmp.data(), (char *) dst->data + i1*nb1 + i02*nb2 + i03*nb3, nc);
            }
        }
    }
}
...
static void ggml_compute_forward_flash_attn_ext_tiled(
    ...
    while (ir < ir1) {
        ...
        for (int64_t ic = 0; ic < nek1; ic += KV_TILE_SZ) {
            ...
            ggml_to_float_t const k_to_float = ggml_get_type_traits(kv_type)->to_float;
            for (int tk = 0; tk < kv_tile; tk++) {
                ...
                } else if (kv_type == GGML_TYPE_F32) {
                    const float * k_f32_src = (const float *)k_data;
                    for (int64_t dk = 0; dk < DK; dk++) {
                        K_f32[dk * KV_TILE_SZ + tk] = k_f32_src[dk];
                    }
                } else if (k_to_float) {
                    // Quantized KV (e.g., turbo2/3/4, q8_0, etc.) — dequantize row to F32
                    std::vector<float> k_row(DK);
                    k_to_float(k_data, k_row.data(), DK);
                    for (int64_t dk = 0; dk < DK; dk++) {
                        K_f32[dk * KV_TILE_SZ + tk] = k_row[dk];
                    }
                }
            }
            ...
            ggml_to_float_t const v_to_float_tiled = ggml_get_type_traits(kv_type)->to_float;
            for (int tk = 0; tk < kv_tile; tk++) {
                ...
                } else if (v_to_float_tiled) {
                    v_to_float_tiled(v_data, V32 + tk * DV, DV);
                }
            }
            for (int tq = 0; tq < Q_TILE_SZ; tq++) {
                if (skip[tq]) {
