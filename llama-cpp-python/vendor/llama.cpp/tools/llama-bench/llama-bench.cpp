...
struct cmd_params {
    ...
    std::vector<uint32_t>            kv_page_size;
...
static void print_usage(int /* argc */, char ** argv) {
    ...
    printf("  --kv-page-size <n>                          paged KV tokens per block (0=flat, default: %s)\n", join(cmd_params_defaults.kv_page_size, ",").c_str());
...
static cmd_params parse_cmd_params(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        try {
            if (arg == "-h" || arg == "--help") {
                ...
                auto p = string_split<uint32_t>(argv[i], split_delim);
                params.kv_page_size.insert(params.kv_page_size.end(), p.begin(), p.end());
    ...
    if (params.kv_page_size.empty()) {
        params.kv_page_size = cmd_params_defaults.kv_page_size;
    }
...
struct cmd_params_instance {
    ...
    llama_context_params to_llama_cparams() const {
        ...
        cparams.kv_page_size    = kv_page_size;
...
static std::vector<cmd_params_instance> get_cmd_params_instances(const cmd_params & params) {
    ...
    for (const auto & kvps : params.kv_page_size)
...
struct test {
    ...
    test(const cmd_params_instance & inst, const llama_model * lmodel, const llama_context * ctx) :
        ...
        kv_page_size   = inst.kv_page_size;
    ...
    static const std::vector<std::string> & get_fields() {
        static const std::vector<std::string> fields = {
            ...
            "main_gpu",       "no_kv_offload",  "flash_attn",    "kv_page_size",   "devices",        "tensor_split",
    ...
    static field_type get_field_type(const std::string & field) {
        if (field == "build_number" || field == "n_batch" || field == "n_ubatch" || field == "n_threads" ||
            ...
            field == "stddev_ns" || field == "no_op_offload" || field == "n_cpu_moe" || field == "kv_page_size") {
            return INT;
        }
...
struct markdown_printer : public printer {
    ...
    void print_header(const cmd_params & params) override {
        ...
        if (params.kv_page_size.size() > 1 || params.kv_page_size != cmd_params_defaults.kv_page_size) {
            fields.emplace_back("kv_page_size");
        }
