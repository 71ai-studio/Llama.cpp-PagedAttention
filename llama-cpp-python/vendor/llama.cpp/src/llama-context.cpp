...
llama_context::llama_context(
    ...
    // Paged KV cache requires flash attention and unified (single-stream) mode
    if (cparams.kv_page_size > 0) {
        if (!cparams.flash_attn) {
            LLAMA_LOG_WARN("%s: kv_page_size > 0 requires flash_attn — enabling automatically\n", __func__);
            cparams.flash_attn = true;
        }
        if (!cparams.kv_unified) {
            LLAMA_LOG_WARN("%s: kv_page_size > 0 requires kv_unified — enabling automatically\n", __func__);
            cparams.kv_unified = true;
        }
        if (cparams.offload_kqv) {
            LLAMA_LOG_WARN("%s: kv_page_size > 0: disabling offload_kqv (paged cache runs on CPU)\n", __func__);
