...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--kv-page-size"}, "N",
        "paged KV cache: tokens per block (0 = flat/disabled, default: 0); requires --flash-attn",
        [](common_params & params, int value) {
            params.kv_page_size = (uint32_t)std::max(0, value);
        }
    ).set_env("LLAMA_ARG_KV_PAGE_SIZE").set_examples({LLAMA_EXAMPLE_BENCH, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_COMPLETION, LLAMA_EXAMPLE_PERPLEXITY}));
    add_opt(common_arg(
        {"--context-shift"},
        {"--no-context-shift"},
