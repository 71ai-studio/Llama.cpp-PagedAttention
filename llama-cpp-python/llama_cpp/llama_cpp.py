...
GGML_TYPE_MXFP4 = 39
GGML_TYPE_NVFP4 = 40
# TurboQuant KV cache types (PolarQuant + QJL)
# Requires llama-cpp-python built with TurboQuant+ patches applied to vendor/llama.cpp
# Usage: Llama(model_path=..., type_k=GGML_TYPE_TURBO3, type_v=GGML_TYPE_TURBO3)
GGML_TYPE_TURBO4 = 41  # 3-bit PolarQuant + 1-bit QJL = ~4 bits/elem, 3.76x compression vs f16
GGML_TYPE_TURBO3 = 42  # 2-bit PolarQuant + 1-bit QJL = ~3 bits/elem, 4.92x compression vs f16
GGML_TYPE_TURBO2 = 43  # 1-bit PolarQuant + 1-bit QJL = ~2 bits/elem, 7.11x compression vs f16
GGML_TYPE_COUNT = 44
...
#     uint32_t kv_page_size; // paged KV cache: tokens per block (0 = disabled, use flat allocation)
#                            // requires flash_attn=true; forces kv_unified=true and offload_kqv=false
...
class llama_context_params(ctypes.Structure):
    ...
    Attributes:
        ...
        kv_page_size (int): paged KV cache block size in tokens (0 = flat/disabled, requires flash_attn)
    ...
    _fields_ = [
        ...
        ("kv_page_size", ctypes.c_uint32),  # Phase 2 paged KV cache
        ("samplers", ctypes.POINTER(llama_sampler_seq_config)),
