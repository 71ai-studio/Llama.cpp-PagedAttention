    ...
    def __init__(
        ...
        **kwargs,  # type: ignore
        ...
        Args:
            ...
            kv_page_size: Paged KV cache block size in tokens (0 = flat/disabled).
                Values: 8, 16, 32. Requires flash_attn=True (auto-enabled).
                Enables prefix caching and block-level eviction (Phase 2).
        ...
        # Paged KV cache (Phase 2) — requires flash_attn=True
        if kv_page_size > 0:
            self.context_params.kv_page_size = kv_page_size
            # kv_page_size forces flash_attn; auto-enable if not set
            if not flash_attn:
                self.context_params.flash_attn_type = llama_cpp.LLAMA_FLASH_ATTN_TYPE_ENABLED
        # Sampling Params
